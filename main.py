...
from src.constants import DOWNLOAD_PATH, INVALID_CHAR, TMP_DOWNLOAD_PATH
...
def concatenate_ts_files(no, output_name, input_dir=tmp_download_path, output_dir=download_path, convert_to_mp4=False):
    ...
    print('[INFO]: Start cleaning tmp files...')
    clean_tmp_dir(tmp_download_path)

    print('[INFO]: Saved the result file at {}.'.format(output_fullname))
        ...
        while not_completed: 
            ...
            for file in listdir(output_dir):
                if '.part' in file:
                    ...
                    not_completed = True
                    if tot_sec > TIMEOUT:
                        print('[WARN]: Failed to download {}.'.format(url))
                        cnt_failed += 1
                        failed = True
                    break
            if failed:
    ...
    if match:
        ...
        while True:
            if 'index.m3u8' not in listdir(tmp_dir):
                ...
                tot_sec += sec
                if tot_sec > TIMEOUT:
                    print('[ERR]: Failed to get index.m3u8.')
                    return None
            else:
                break
        ...
        with open(m3u8_path, 'r') as file:
            ...
            for line in m3u8[::-1]:
                ...
                if match:
                    ...
                    print('[INFO]: # of M3U8 files: {}.'.format(no))
                    break

            if no < 0:
                print('[ERR]: Failed to find the number of M3U8 files.')
                return None
            
        return [url_root, [URL_SUBSTR.format(i) for i in range(1, no + 1)], url_query]
    else:
        print('[ERR]: Failed to parse the root url from {}.'.format(m3u8))
        return None

if __name__ == '__main__':
    assert len(sys.argv) == 2, '[ERR]: 2 args (including filename) are needed, but {} given.'.format(len(sys.argv))
    ...
    print('[INFO]: Start getting M3U8 URL...')
    html = requests.get(VID_PATH).text
        ...
        if match:
            ...
            for quality in quality_lists:
                if str(best_quality) in quality['source_label']:
                    ...
                    break
    print('[INFO]: Found: {}. Best quality: {}p, (URL: {}).'.format(title, best_quality, m3u8))

    print('[INFO]: Start parsing M3U8...')
    [url_root, urls, url_query] = parse_m3u8(m3u8)
    if url_root and urls and url_query:
        print('[INFO]: Start downloading ts files...')
        completed = download_ts_files(url_root, urls, url_query)
        
        diff = len(urls) - completed
        print('[INFO]: Total: {}. Completed: {}, failed: {}.'.format(len(urls), completed, diff))
        if diff == 0:
            filename = ''.join(char for char in title if char not in INVALID_CHAR)

            print('[INFO]: Start concantenating ts files...')
            ...
            print('[WARN]: Stop concantenating ts files...')
    else:
        print('[WARN]: Terminating the task...')
